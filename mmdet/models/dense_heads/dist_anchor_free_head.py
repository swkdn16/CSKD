...
class dist_AnchorFreeHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
                 ...
                 init_cfg=dict(
                     ...
                     override=dict(
                         ...
                         std=0.01,
                         bias_prob=0.01))):
        super(dist_AnchorFreeHead, self).__init__(init_cfg)
    ...
    def forward_single(self, x):
        ...
        if self.training or self.is_teacher:
            ...
            for cls_layer in self.cls_convs:
                ...
                _cls_feat.append(cls_feat)
            cls_score = self.conv_cls(cls_feat)
            for reg_layer in self.reg_convs:
                reg_feat = reg_layer(reg_feat)
                _reg_feat.append(reg_feat)
            bbox_pred = self.conv_reg(reg_feat)
            return cls_score, bbox_pred, cls_feat, reg_feat, _cls_feat, _reg_feat
        else:
            for cls_layer in self.cls_convs:
                cls_feat = cls_layer(cls_feat)
            cls_score = self.conv_cls(cls_feat)
